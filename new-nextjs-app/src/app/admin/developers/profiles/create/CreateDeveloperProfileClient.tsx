...
const CreateDeveloperProfileClient = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // First, create the developer profile with basic data
      const profileData = {
      ...
      const response = await http.post('/api/v1/admin/developers/profiles', profileData);
      const developerId = response.data.data._id;

      // Upload logo if exists
      if (logoFile && developerId) {
        const logoFormData = new FormData();
        logoFormData.append('logo', logoFile);
        await http.put(`/api/v1/developers/${developerId}/logo`, logoFormData, {
          headers: {
      ...
      setSnackbarMessage('Developer profile created successfully!');
...
export default CreateDeveloperProfileClient;
