...
const InquiriesManagement = () => {
  ...
  const { data, isLoading } = useQuery(['inquiries', page, rowsPerPage, searchTerm], async () => {
    const res = await axios.get(`/api/v1/admin/inquiries?page=${page + 1}&limit=${rowsPerPage}&search=${searchTerm}`);
    return res.data;
  });

  const deleteInquiry = useMutation((inquiryId) => axios.delete(`/api/v1/admin/inquiries/${inquiryId}`), {
    onSuccess: () => {
  ...
  const markAsRead = useMutation((inquiryId) => axios.patch(`/api/v1/admin/inquiries/${inquiryId}/read`), {
    onSuccess: () => {
      queryClient.invalidateQueries('inquiries');
    }
  });
              ...
              >
                ...
                <TableCell>
                  <IconButton onClick={(e) => {
                    e.stopPropagation();
                    deleteInquiry.mutate(inquiry._id);
                  }}>
